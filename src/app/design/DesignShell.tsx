  ...
  return (
    <div className="h-screen flex flex-col">
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <aside
          className={`
            fixed inset-y-0 start-0 z-30 w-64 transform transition-transform duration-200 ease-out
            lg:relative lg:translate-x-0 lg:z-auto lg:shrink-0
            ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}
            flex flex-col border-e border-[var(--card-border)]
            bg-[var(--background)]
          `}
        >
          {/* Desktop header */}
          ...
          <div className="shrink-0 px-3 pb-3">
            <div className="relative">
              <MaterialSymbol className="absolute start-2.5 top-1/2 -translate-y-1/2 text-[16px] opacity-40 pointer-events-none">search</MaterialSymbol>
              <input
                ref={searchRef}
                type="search"
                value={searchQuery}
                ...
                className="w-full ps-8 pe-8 py-1.5 text-sm rounded-lg bg-foreground/[0.06] border border-[var(--card-border)] placeholder:opacity-40 focus:outline-none focus:ring-1 focus:ring-primary/50 focus:border-primary/50 transition-colors"
                aria-label="Search design system sections"
              />
              {searchQuery && (
                <button
                  onClick={() => setSearchQuery('')}
                  className="absolute end-2 top-1/2 -translate-y-1/2 text-[14px] opacity-40 hover:opacity-80 transition-opacity"
