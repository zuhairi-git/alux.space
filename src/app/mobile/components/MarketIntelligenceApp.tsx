    ...
    return (
        <div className={`flex flex-col h-full w-full relative ${bgClass} transition-colors duration-500 font-sans`}>
                ...
                <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
                    {isColorful ? (
                        <>
                            ...
                            <div className={`absolute top-1/3 -right-16 w-48 h-48 ${isColorful ? 'bg-ds-purple-400/8' : 'bg-[var(--primary)]/8'} rounded-full blur-3xl animate-pulse`} style={{ animationDuration: '8s', animationDelay: '2s' }} />
                        ...
                        <>
                            ...
                            <div className={`absolute bottom-20 -right-16 w-48 h-48 ${isColorful ? 'bg-ds-purple-400/[0.04]' : 'bg-[var(--primary)]/[0.04]'} rounded-full blur-3xl animate-pulse`} style={{ animationDuration: '10s', animationDelay: '3s' }} />
                        </>
                    )}
                </div>
            )}
            ...
            <header className={`absolute top-0 w-full pt-12 pb-3 px-5 z-40 transition-all duration-300 ${isIOS
                ? (isLight ? 'bg-white/40 backdrop-blur-[30px] backdrop-saturate-[200%] border-b border-black/5' : isColorful ? 'bg-ds-card-colorful-from/50 backdrop-blur-[30px] backdrop-saturate-[200%] border-b border-ds-purple-400/15' : 'bg-ds-dark-3/45 backdrop-blur-[30px] backdrop-saturate-[200%] border-b border-white/5')
                : (isLight ? 'bg-ds-gray-50/90 backdrop-blur-2xl border-b border-ds-blue-100/50 shadow-sm' : isColorful ? 'bg-ds-card-colorful-from/80 backdrop-blur-2xl border-b border-ds-purple-400/20' : 'bg-ds-dark-3/90 backdrop-blur-2xl border-b border-ds-gray-600/50 shadow-md')
                }`}>
...
function DashboardView({ os, theme, onNavigate }: { os: string, theme: string, onNavigate: (t: TabType) => void }) {
    ...
    const card = isIOS
        ? (isLight ? 'bg-white/50 backdrop-blur-[30px] backdrop-saturate-[200%] border border-white/50 rounded-[22px] shadow-[0_8px_32px_rgba(0,0,0,0.04)]' : isColorful ? 'bg-ds-card-colorful-from/35 backdrop-blur-[30px] backdrop-saturate-[200%] border border-ds-purple-400/20 rounded-[22px] shadow-[0_8px_32px_rgba(0,0,0,0.3)]' : 'bg-ds-dark-2/55 backdrop-blur-[30px] backdrop-saturate-[200%] border border-white/8 rounded-[22px] shadow-[0_8px_32px_rgba(0,0,0,0.3)]')
        : (isLight ? 'bg-ds-gray-50/90 backdrop-blur-xl rounded-[28px] shadow-sm border border-ds-blue-100/50' : isColorful ? 'bg-ds-card-colorful-from/60 backdrop-blur-xl rounded-[28px] shadow-lg border border-ds-purple-400/20' : 'bg-ds-dark-3/90 backdrop-blur-xl rounded-[28px] shadow-lg border border-ds-gray-600/40');
    ...
    return (
        <motion.div initial="hidden" animate="show" variants={stagger} className="absolute inset-0 overflow-y-auto scrollbar-none pb-28 pt-[110px] px-4 space-y-5">
            ...
            <motion.div variants={fadeUp} className={`relative overflow-hidden p-5 ${card}`}>
                ...
                <div className={`absolute top-0 left-6 right-6 h-[2px] rounded-full bg-gradient-to-r opacity-70 ${isColorful ? 'from-ds-purple-400 to-ds-purple-700' : isIOS ? 'from-ds-blue-500 to-ds-indigo-500' : 'from-ds-blue-500 to-ds-blue-300'}`} />
                    ...
                    <div className="flex space-x-1">
                        {['1D', '1W', '1M'].map(t => (
                            <button key={t} onClick={() => setSelectedTimeRange(t)}
                                className={`px-2.5 py-1 rounded-full text-[10px] font-bold transition-all ${selectedTimeRange === t ? (isIOS ? 'bg-ds-blue-500 text-white' : isColorful ? 'bg-ds-purple-400 text-white' : 'bg-ds-blue-500 text-white') : (isLight ? 'bg-black/5 text-gray-500' : 'bg-white/8 text-gray-400')}`}>
                                {t}
                            </button>
                        ))}
                    </div>
            ...
            <motion.div variants={fadeUp} className={`relative overflow-hidden p-5 ${card}`}>
                ...
                <div className="pl-3">
                    <div className="flex items-center space-x-2.5 mb-3">
                        <Icon name="auto_awesome" className={`text-xl ${isColorful ? 'text-ds-purple-400' : 'text-ds-blue-500'}`} />
                        <span className={`text-xs font-bold uppercase tracking-widest ${isColorful ? 'text-ds-purple-300' : isLight ? 'text-ds-blue-500' : isIOS ? 'text-ds-blue-400' : 'text-ds-blue-300'}`}>AI Morning Briefing</span>
                    </div>
                    ...
                    <button onClick={() => onNavigate('copilot')} className={`mt-3 flex items-center space-x-1.5 text-xs font-semibold ${isColorful ? 'text-ds-purple-300' : 'text-ds-blue-500'}`}>
            ...
            <motion.div variants={fadeUp}>
                <div className="flex justify-between items-center mb-3 px-1">
                    ...
                    <button className={`text-xs font-semibold ${isColorful ? 'text-ds-purple-300' : 'text-ds-blue-500'}`}>See All</button>
                </div>
            ...
            <motion.div variants={fadeUp} className={`p-5 ${card}`}>
                <div className="flex items-center justify-between mb-3">
                    ...
                    <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full flex items-center space-x-1 ${isColorful ? 'bg-ds-purple-400/20 text-ds-purple-300' : isLight ? 'bg-ds-blue-100 text-ds-blue-500' : 'bg-ds-blue-500/10 text-ds-blue-400'}`}>
            ...
            <motion.div variants={fadeUp}>
                ...
                <div className="grid grid-cols-2 gap-3">
                    {[
                        ...
                        { icon: 'grid_view', label: 'Sectors', desc: 'Heatmap', gradient: 'from-ds-purple-400 to-ds-purple-700', lightBg: 'bg-ds-purple-400/10 border-ds-purple-400/20' },
                    ].map((a) => (
    ...
    return (
        <motion.div initial="hidden" animate="show" variants={stagger} className="absolute inset-0 overflow-y-auto scrollbar-none pb-28 pt-[110px] px-4 space-y-5">
            ...
            <motion.div variants={fadeUp} className={`p-5 ${card} relative overflow-hidden`}>
                <div className={`absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r opacity-50 ${isColorful ? 'from-ds-purple-400 to-ds-purple-700' : isIOS ? 'from-ds-blue-500 to-ds-indigo-500' : 'from-ds-blue-500 to-ds-blue-300'}`} />
                    ...
                    <div className="flex space-x-1.5">
                        {['1D', '1W', '1M', '3M', '1Y'].map(r => (
                            <button key={r} onClick={() => setChartRange(r)}
                                className={`px-3 py-1.5 rounded-full text-[11px] font-bold transition-all ${chartRange === r ? (isColorful ? 'bg-ds-purple-400 text-white' : 'bg-ds-blue-500 text-white') : (isLight ? 'bg-black/5 text-gray-500' : 'bg-white/8 text-gray-400')}`}>
                                {r}
                            </button>
                        ))}
                    </div>
                    ...
                    <div className="flex space-x-1.5">
                        {trending.slice(0, 4).map((t, i) => (
                            <button key={t.ticker} onClick={() => setFeaturedIdx(i)}
                                className={`w-6 h-6 rounded-full text-[8px] font-bold flex items-center justify-center transition-all ${featuredIdx === i ? (isColorful ? 'bg-ds-purple-400 text-white scale-110' : 'bg-ds-blue-500 text-white scale-110') : (isLight ? 'bg-black/5 text-gray-500' : 'bg-white/8 text-gray-400')}`}>
                                {t.ticker.slice(0, 2)}
                            </button>
                        ))}
                    </div>
                                ...
                                <div className="px-6">
                                        ...
                                        {tickerExpanded && (
                                            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
                                                ...
                                                <div className={`p-3 rounded-2xl mb-4 ${isLight ? 'bg-black/[0.04]' : 'bg-white/[0.05]'}`}>
                                                    <div className="flex items-center space-x-1.5 mb-1"><Icon name="auto_awesome" className={`text-[14px] ${isColorful ? 'text-ds-purple-400' : 'text-ds-blue-500'}`} /><span className="text-[11px] font-bold opacity-50">AI Insight</span></div>
                                    ...
                                    <div className="grid grid-cols-3 gap-3 mt-4">
                                        {['Ask AI', 'Add Alert', 'Share'].map((label, i) => (
                                            ...
                                            }} className={`py-3 rounded-2xl text-[13px] font-semibold transition-all active:scale-95 ${i === 0 ? (isColorful ? 'bg-ds-purple-400 text-white' : 'bg-ds-blue-500 text-white') : label === 'Add Alert' && alertSet === t.ticker ? 'bg-ds-success text-white' : (isLight ? 'bg-gray-100 text-gray-800' : isColorful ? 'bg-white/10 text-white' : 'bg-white/10 text-white')}`}>
...
function CopilotView({ os, theme }: { os: string, theme: string }) {
    ...
    const userBubble = isColorful
        ? 'bg-gradient-to-br from-ds-purple-400 to-ds-purple-700 text-white rounded-[22px] rounded-tr-md shadow-[0_6px_20px_rgb(var(--glow-primary-rgb),0.3)]'
        : isIOS
    ...
    const botBubble = isIOS
        ? (isLight ? 'bg-white/50 backdrop-blur-[30px] backdrop-saturate-[200%] border border-white/50 text-black rounded-[22px] rounded-tl-md shadow-[0_4px_20px_rgba(0,0,0,0.04)]' : isColorful ? 'bg-ds-card-colorful-from/50 backdrop-blur-[30px] backdrop-saturate-[200%] border border-ds-purple-400/20 text-white rounded-[22px] rounded-tl-md shadow-[0_4px_20px_rgba(0,0,0,0.3)]' : 'bg-ds-dark-2/55 backdrop-blur-[30px] backdrop-saturate-[200%] border border-white/8 text-white rounded-[22px] rounded-tl-md shadow-[0_4px_20px_rgba(0,0,0,0.3)]')
        : isColorful ? (isLight ? 'bg-[#ede9fe]/90 backdrop-blur-xl border border-ds-purple-400/20 text-ds-purple-700 rounded-[22px] rounded-tl-sm shadow-sm' : 'bg-ds-card-colorful-from/60 backdrop-blur-xl border border-ds-purple-400/20 text-white rounded-[22px] rounded-tl-sm shadow-sm') : (isLight ? 'bg-ds-gray-50/90 backdrop-blur-xl border border-ds-blue-100/50 text-ds-blue-900 rounded-[22px] rounded-tl-sm shadow-sm' : 'bg-ds-dark-3/90 backdrop-blur-xl border border-ds-gray-600/40 text-ds-blue-200 rounded-[22px] rounded-tl-sm shadow-sm');
    const inputArea = isIOS
        ? (isLight ? 'bg-white/40 backdrop-blur-[30px] backdrop-saturate-[200%] border-t border-black/5' : isColorful ? 'bg-ds-card-colorful-from/50 backdrop-blur-[30px] backdrop-saturate-[200%] border-t border-ds-purple-400/20' : 'bg-ds-dark-2/45 backdrop-blur-[30px] backdrop-saturate-[200%] border-t border-white/8')
    ...
    const inputBox = isIOS
        ? (isLight ? 'bg-black/5 rounded-full px-5 py-3 border border-black/5' : isColorful ? 'bg-ds-purple-400/10 rounded-full px-5 py-3 border border-ds-purple-400/20' : 'bg-white/8 rounded-full px-5 py-3 border border-white/8')
        : isColorful ? (isLight ? 'bg-ds-purple-400/20 text-ds-purple-700 rounded-[28px] px-5 py-3' : 'bg-ds-purple-400/15 text-white rounded-[28px] px-5 py-3') : (isLight ? 'bg-ds-blue-100 text-ds-blue-900 rounded-[28px] px-5 py-3' : 'bg-ds-blue-800 text-ds-blue-200 rounded-[28px] px-5 py-3');
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.25 }}
                ...
                <div className="flex-1 flex flex-col p-6 items-center overflow-y-auto scrollbar-none">
                    <motion.div initial={{ scale: 0.7, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.1, type: 'spring', stiffness: 250, damping: 20, mass: 0.9 }}
                        ...
                        <div className="relative w-16 h-16 flex items-center justify-center mb-6">
                            <div className={`absolute inset-0 rounded-full animate-ping opacity-15 ${isColorful ? 'bg-ds-purple-400' : 'bg-ds-blue-500'}`} style={{ animationDuration: '2s', animationIterationCount: 2, animationFillMode: 'forwards' }} />
                            <Icon name="auto_awesome" className={`text-[36px] ${isColorful ? 'text-ds-purple-400' : 'text-ds-blue-500'}`} />
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-2.5">
                            {prompts.map((p, i) => (
                                <motion.button key={p.label} whileTap={{ scale: 0.93 }} onClick={() => handleSend(p.prompt)}
                                    ...
                                    className={`flex flex-col text-left p-3.5 rounded-[18px] transition-all ${isIOS ? (isLight ? 'bg-white/60 border border-black/5' : isColorful ? 'bg-ds-card-colorful-from/60 border border-ds-purple-400/20' : 'bg-ds-dark-2/60 border border-white/8') : isColorful ? (isLight ? 'bg-ds-purple-400/15 border border-ds-purple-400/20' : 'bg-ds-purple-400/10 border border-ds-purple-400/20') : (isLight ? 'bg-ds-blue-100 text-ds-blue-900' : 'bg-ds-blue-800 text-ds-blue-200')}`}>
                                    <Icon name={p.icon} className={`mb-1.5 text-lg ${isColorful ? 'text-ds-purple-400' : isIOS ? (isLight ? 'text-ds-blue-500' : 'text-ds-blue-400') : (isLight ? 'text-ds-blue-500' : 'text-ds-blue-400')}`} />
                                    ...
                                    <div className="mt-2 space-y-1.5 ml-1">
                                        {msg.citations.map((c, i) => (
                                            <motion.div key={i} initial={{ opacity: 0, x: -20, scale: 0.9 }} animate={{ opacity: 1, x: 0, scale: 1 }} transition={{ delay: 0.15 + i * 0.12, type: 'spring', stiffness: 280, damping: 22 }}
                                                className={`flex items-start space-x-2 p-2.5 rounded-xl text-[11px] ${isLight ? 'bg-ds-blue-400/8 border border-ds-blue-400/10' : isColorful ? 'bg-ds-purple-400/8 border border-ds-purple-400/15' : 'bg-ds-indigo-600/20 border border-ds-indigo-500/15'}`}>
                                                <Icon name="verified" className={`text-sm shrink-0 mt-0.5 ${isColorful ? 'text-ds-purple-400' : 'text-ds-blue-500'}`} />
                        ...
                        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="flex flex-wrap gap-2 px-1 pb-2">
                            {[
                                ...
                                <motion.button key={a.label} whileTap={{ scale: 0.95 }}
                                    className={`flex items-center space-x-1.5 px-3 py-2 rounded-2xl text-[12px] font-semibold ${isColorful ? (isLight ? 'bg-ds-purple-400/20 text-ds-purple-700' : 'bg-ds-purple-400/15 text-ds-purple-300') : isIOS ? (isLight ? 'bg-black/5 text-ds-blue-500' : 'bg-white/8 text-ds-blue-400') : (isLight ? 'bg-ds-blue-100 text-ds-blue-900' : 'bg-ds-blue-800 text-ds-blue-200')}`}>
            ...
            <div className={`px-4 pt-3 pb-[90px] flex items-end space-x-2 shrink-0 z-30 w-full ${inputArea}`}>
                ...
                <button onClick={() => handleSend()} disabled={!input.trim()} className={`w-10 h-10 flex justify-center items-center shrink-0 transition-opacity disabled:opacity-40 ${isColorful ? 'bg-ds-purple-400 text-white rounded-full' : isIOS ? 'bg-ds-blue-500 text-white rounded-full' : 'bg-ds-blue-500 text-white rounded-[14px]'}`}>
...
function AlertsView({ os, theme }: { os: string, theme: string }) {
    ...
    const card = isIOS
        ? (isLight ? 'bg-white/50 backdrop-blur-[30px] backdrop-saturate-[200%] border border-white/50 rounded-[22px] shadow-[0_4px_20px_rgba(0,0,0,0.04)]' : isColorful ? 'bg-ds-card-colorful-from/35 backdrop-blur-[30px] backdrop-saturate-[200%] border border-ds-purple-400/20 rounded-[22px] shadow-[0_4px_20px_rgba(0,0,0,0.3)]' : 'bg-ds-dark-2/55 backdrop-blur-[30px] backdrop-saturate-[200%] border border-white/8 rounded-[22px] shadow-[0_4px_20px_rgba(0,0,0,0.3)]')
        : (isLight ? 'bg-ds-gray-50/90 backdrop-blur-xl rounded-[28px] border border-ds-blue-100/50 shadow-sm' : isColorful ? 'bg-ds-card-colorful-from/60 backdrop-blur-xl rounded-[28px] shadow-lg border border-ds-purple-400/20' : 'bg-ds-dark-3/90 backdrop-blur-xl rounded-[28px] border border-ds-gray-600/40 shadow-lg');
    ...
    const priorityBadge = (p: string) => {
        ...
        return isColorful ? (isLight ? 'bg-ds-purple-400/20 text-ds-purple-700 border-ds-purple-400/30' : 'bg-ds-purple-400/15 text-ds-purple-400 border-ds-purple-400/30') : isLight ? 'bg-ds-blue-100 text-ds-blue-500 border-ds-blue-200' : 'bg-ds-blue-500/10 text-ds-blue-400 border-ds-blue-500/15';
    };
    ...
    return (
        <motion.div initial="hidden" animate="show" variants={stagger} className="absolute inset-0 overflow-y-auto scrollbar-none pb-28 pt-[110px] px-4 space-y-0">
            ...
            <motion.div variants={fadeUp} className="flex justify-between items-center px-1 mb-4">
                <h2 className={`${isIOS ? 'text-2xl font-bold tracking-tight' : isColorful ? 'text-xl font-medium text-ds-purple-300' : 'text-xl font-medium text-ds-blue-500'}`}>Market Alerts</h2>
                <div className="flex items-center space-x-2">
                                        ...
                                        <AnimatePresence>
                                            {expanded === a.id && (
                                                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ type: 'spring', stiffness: 340, damping: 30, mass: 0.9 }}>
                                                    ...
                                                    <button className={`mt-3 flex items-center space-x-1.5 text-xs font-semibold ${isColorful ? 'text-ds-purple-300' : 'text-ds-blue-500'}`}>
    ...
    return (
        <motion.div initial="hidden" animate="show" variants={stagger} className="absolute inset-0 overflow-y-auto scrollbar-none pb-28 pt-[110px] px-4 space-y-5">
            ...
            <motion.div variants={fadeUp} className="flex flex-col items-center py-4">
                ...
                <p className={`text-sm font-medium mt-0.5 ${isColorful ? 'text-ds-purple-300' : 'text-ds-blue-500'}`}>Alux Space Founder</p>
            </motion.div>
            ...
            <motion.div variants={fadeUp} className="grid grid-cols-3 gap-2">
                {[
                    ...
                    <div key={stat.label} className={`p-3 text-center ${card}`}>
                        <Icon name={stat.icon} className={`text-[18px] mb-1 ${isColorful ? 'text-ds-purple-400' : 'text-ds-blue-500'}`} />
            ...
            <motion.div variants={fadeUp} className={`p-5 ${card} relative overflow-hidden`}>
                <div className={`absolute top-0 left-6 right-6 h-[2px] rounded-full bg-gradient-to-r opacity-50 ${isColorful ? 'from-ds-purple-400 to-ds-purple-700' : isIOS ? 'from-ds-blue-500 to-ds-indigo-500' : 'from-ds-blue-500 to-ds-blue-300'}`} />
                <div className="flex justify-between items-start mb-4">
                    ...
                    {[40, 65, 45, 80, 55, 90, 75].map((h, i) => (
                        <div key={i} className="w-full h-full relative group flex items-end justify-center z-10">
                            <motion.div initial={{ height: 0 }} animate={{ height: `${h}%` }} transition={{ type: 'spring', stiffness: 200, damping: 20, mass: 0.8, delay: i * 0.06 }}
                                className={`w-full max-w-[10px] rounded-t-full bg-gradient-to-t ${isColorful ? ['from-ds-purple-400/60 to-ds-purple-400', 'from-ds-purple-400/70 to-ds-purple-700', 'from-ds-purple-400/50 to-ds-purple-400', 'from-ds-purple-400/80 to-ds-purple-700', 'from-ds-purple-400/65 to-ds-purple-400', 'from-ds-purple-400 to-ds-purple-700', 'from-ds-purple-400/90 to-ds-purple-700'][i] : ['from-ds-blue-400 to-ds-blue-600', 'from-ds-indigo-400 to-ds-indigo-600', 'from-ds-blue-300 to-ds-blue-500', 'from-ds-blue-500 to-ds-indigo-500', 'from-ds-indigo-400 to-ds-blue-600', 'from-ds-blue-400 to-ds-indigo-500', 'from-ds-indigo-500 to-ds-blue-500'][i]}`} />
                        </div>
                    ))}
                </div>
                {/* Day labels */}
                ...
                {isThemeModalOpen && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-auto">
                        ...
                        <motion.div initial={{ opacity: 0, scale: 0.7, y: 40 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.7, y: 40 }}
                            ...
                            <div className="space-y-2.5">
                                {[{ value: 'light', label: 'Light', icon: 'light_mode', color: 'text-ds-purple-400' }, { value: 'dark', label: 'Dark', icon: 'dark_mode', color: 'text-ds-blue-400' }, { value: 'colorful', label: 'Colorful', icon: 'palette', color: 'text-ds-purple-400' }].map(t => {
...
function BottomNav({ activeTab, setActiveTab, os, theme }: { activeTab: string, setActiveTab: (t: TabType) => void, os: string, theme: string }) {
    ...
    const navClass = isIOS
        ? (isLight ? 'bg-white/40 backdrop-blur-[30px] backdrop-saturate-[200%] border-t border-black/5 pb-5 h-[82px]' : isColorful ? 'bg-ds-card-colorful-from/50 backdrop-blur-[30px] backdrop-saturate-[200%] border-t border-ds-purple-400/15 pb-5 h-[82px]' : 'bg-ds-dark-2/45 backdrop-blur-[30px] backdrop-saturate-[200%] border-t border-white/5 pb-5 h-[82px]')
        : (isLight ? 'bg-ds-gray-100 h-20 pb-2 border-t border-ds-blue-100/50' : isColorful ? 'bg-ds-card-colorful-from/80 h-20 pb-2 border-t border-ds-purple-400/20' : 'bg-ds-dark-3 h-20 pb-2 border-t border-ds-gray-600/50');
...
function NavBtn({ icon, label, active, onClick, os, theme }: { icon: string, label: string, active: boolean, onClick: () => void, os: string, theme: string }) {
    ...
    const isColorful = theme === 'colorful';
    if (isIOS) {
        const activeColor = isColorful ? 'text-ds-purple-400' : 'text-ds-blue-500';
        return (
    ...
    const iconColor = active ? (isLight ? 'text-ds-blue-900' : isColorful ? 'text-ds-purple-300' : 'text-ds-blue-200') : (isLight ? 'text-ds-gray-600' : 'text-ds-gray-300');
    return (
            ...
            <div className="relative w-16 h-8 rounded-full flex items-center justify-center">
                {active && (
                    <motion.div
                        ...
                        className={`absolute inset-0 rounded-full ${isColorful ? (isLight ? 'bg-ds-purple-400/25' : 'bg-ds-purple-400/25') : (isLight ? 'bg-ds-blue-100' : 'bg-ds-blue-700')}`}
