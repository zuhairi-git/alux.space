...
function PulseBeacon({ color = 'green', size = 'sm' }: { color?: string, size?: 'sm' | 'md' }) {
    ...
    const colorMap: Record<string, string> = { green: 'bg-ds-success', red: 'bg-ds-error', amber: 'bg-ds-warning', blue: 'bg-ds-blue-500', purple: 'bg-primary' };
    return (
    ...
    return (
        <div className={`flex flex-col h-full w-full relative ${bgClass} transition-colors duration-500 font-sans`}>
            ...
            <header className={`absolute top-0 w-full pt-12 pb-3 px-5 z-40 transition-all duration-300 ${isIOS
                ...
                : (isLight ? 'bg-ds-gray-50/90 backdrop-blur-2xl border-b border-ds-blue-100/50 shadow-sm' : isColorful ? 'bg-ds-card-colorful-from/80 backdrop-blur-2xl border-b border-primary/20' : 'bg-ds-dark-3/90 backdrop-blur-2xl border-b border-ds-gray-600/50 shadow-md')
                }`}>
...
function DashboardView({ os, theme, onNavigate }: { os: string, theme: string, onNavigate: (t: TabType) => void }) {
    ...
    const card = isIOS
        ...
        : (isLight ? 'bg-ds-gray-50/90 backdrop-blur-xl rounded-[28px] shadow-sm border border-ds-blue-100/50' : isColorful ? 'bg-ds-card-colorful-from/60 backdrop-blur-xl rounded-[28px] shadow-lg border border-primary/20' : 'bg-ds-dark-3/90 backdrop-blur-xl rounded-[28px] shadow-lg border border-ds-gray-600/40');
    ...
    return (
        <motion.div initial="hidden" animate="show" variants={stagger} className="absolute inset-0 overflow-y-auto scrollbar-none pb-28 pt-[110px] px-4 space-y-5">
            ...
            <motion.div variants={fadeUp} className={`relative overflow-hidden p-5 ${card}`}>
                ...
                <div className={`absolute top-0 left-6 right-6 h-[2px] rounded-full bg-gradient-to-r opacity-70 ${isColorful ? 'from-primary to-primary-dark' : isIOS ? 'from-ds-blue-500 to-ds-indigo-500' : 'from-ds-blue-500 to-ds-blue-300'}`} />
            ...
            <motion.div variants={fadeUp}>
                <div className="flex justify-between items-center mb-3 px-1">
                    ...
                    <button className={`text-xs font-semibold ${isColorful ? 'text-accent' : 'text-ds-blue-500'}`}>See All</button>
                </div>
            ...
            <motion.div variants={fadeUp} className={`p-5 ${card}`}>
                <div className="flex items-center justify-between mb-3">
                    ...
                    <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full flex items-center space-x-1 ${isColorful ? 'bg-primary/20 text-accent' : isLight ? 'bg-ds-blue-100 text-ds-blue-500' : 'bg-ds-blue-500/10 text-ds-blue-400'}`}>
    ...
    return (
        <motion.div initial="hidden" animate="show" variants={stagger} className="absolute inset-0 overflow-y-auto scrollbar-none pb-28 pt-[110px] px-4 space-y-5">
            ...
            <motion.div variants={fadeUp} className={`p-5 ${card} relative overflow-hidden`}>
                <div className={`absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r opacity-50 ${isColorful ? 'from-primary to-primary-dark' : isIOS ? 'from-ds-blue-500 to-ds-indigo-500' : 'from-ds-blue-500 to-ds-blue-300'}`} />
...
function CopilotView({ os, theme }: { os: string, theme: string }) {
    ...
    const inputBox = isIOS
        ...
        : isColorful ? (isLight ? 'bg-primary/20 text-primary-dark rounded-[28px] px-5 py-3' : 'bg-primary/15 text-white rounded-[28px] px-5 py-3') : (isLight ? 'bg-ds-blue-100 text-ds-blue-900 rounded-[28px] px-5 py-3' : 'bg-ds-blue-800 text-ds-blue-200 rounded-[28px] px-5 py-3');
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.25 }}
                ...
                <div className="flex-1 flex flex-col p-6 items-center overflow-y-auto scrollbar-none">
                    <motion.div initial={{ scale: 0.7, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.1, type: 'spring', stiffness: 250, damping: 20, mass: 0.9 }}
                        ...
                        <div className="relative w-16 h-16 flex items-center justify-center mb-6">
                            <div className={`absolute inset-0 rounded-full animate-ping opacity-15 ${isColorful ? 'bg-primary' : 'bg-ds-blue-500'}`} style={{ animationDuration: '2s', animationIterationCount: 2, animationFillMode: 'forwards' }} />
                        ...
                        <div className="grid grid-cols-2 gap-2.5">
                            {prompts.map((p, i) => (
                                <motion.button key={p.label} whileTap={{ scale: 0.93 }} onClick={() => handleSend(p.prompt)}
                                    ...
                                    <Icon name={p.icon} className={`mb-1.5 text-lg ${isColorful ? 'text-primary' : isIOS ? (isLight ? 'text-ds-blue-500' : 'text-ds-blue-400') : (isLight ? 'text-ds-blue-500' : 'text-ds-blue-400')}`} />
            ...
            <div className={`px-4 pt-3 pb-[90px] flex items-end space-x-2 shrink-0 z-30 w-full ${inputArea}`}>
                ...
                <button onClick={() => handleSend()} disabled={!input.trim()} className={`w-10 h-10 flex justify-center items-center shrink-0 transition-opacity disabled:opacity-40 ${isColorful ? 'bg-primary text-white rounded-full' : isIOS ? 'bg-ds-blue-500 text-white rounded-full' : 'bg-ds-blue-500 text-white rounded-[14px]'}`}>
...
function AlertsView({ os, theme }: { os: string, theme: string }) {
    ...
    const card = isIOS
        ...
        : (isLight ? 'bg-ds-gray-50/90 backdrop-blur-xl rounded-[28px] border border-ds-blue-100/50 shadow-sm' : isColorful ? 'bg-ds-card-colorful-from/60 backdrop-blur-xl rounded-[28px] shadow-lg border border-primary/20' : 'bg-ds-dark-3/90 backdrop-blur-xl rounded-[28px] border border-ds-gray-600/40 shadow-lg');
    ...
    const priorityBadge = (p: string) => {
        ...
        return isColorful ? (isLight ? 'bg-primary/20 text-primary-dark border-primary/30' : 'bg-primary/15 text-primary border-primary/30') : isLight ? 'bg-ds-blue-100 text-ds-blue-500 border-ds-blue-200' : 'bg-ds-blue-500/10 text-ds-blue-400 border-ds-blue-500/15';
    };
    ...
    return (
        <motion.div initial="hidden" animate="show" variants={stagger} className="absolute inset-0 overflow-y-auto scrollbar-none pb-28 pt-[110px] px-4 space-y-0">
            ...
            <motion.div variants={fadeUp} className="flex justify-between items-center px-1 mb-4">
                <h2 className={`${isIOS ? 'text-2xl font-bold tracking-tight' : isColorful ? 'text-xl font-medium text-accent' : 'text-xl font-medium text-ds-blue-500'}`}>Market Alerts</h2>
                <div className="flex items-center space-x-2">
    ...
    return (
        <motion.div initial="hidden" animate="show" variants={stagger} className="absolute inset-0 overflow-y-auto scrollbar-none pb-28 pt-[110px] px-4 space-y-5">
            ...
            <motion.div variants={fadeUp} className="flex flex-col items-center py-4">
                ...
                <p className={`text-sm font-medium mt-0.5 ${isColorful ? 'text-accent' : 'text-ds-blue-500'}`}>Alux Space Founder</p>
            </motion.div>
            ...
            <motion.div variants={fadeUp} className={`p-5 ${card} relative overflow-hidden`}>
                <div className={`absolute top-0 left-6 right-6 h-[2px] rounded-full bg-gradient-to-r opacity-50 ${isColorful ? 'from-primary to-primary-dark' : isIOS ? 'from-ds-blue-500 to-ds-indigo-500' : 'from-ds-blue-500 to-ds-blue-300'}`} />
                <div className="flex justify-between items-start mb-4">
                    ...
                    {[40, 65, 45, 80, 55, 90, 75].map((h, i) => (
                        <div key={i} className="w-full h-full relative group flex items-end justify-center z-10">
                            <motion.div initial={{ height: 0 }} animate={{ height: `${h}%` }} transition={{ type: 'spring', stiffness: 200, damping: 20, mass: 0.8, delay: i * 0.06 }}
                                className={`w-full max-w-[10px] rounded-t-full bg-gradient-to-t ${isColorful ? ['from-primary/60 to-primary', 'from-primary/70 to-primary-dark', 'from-primary/50 to-primary', 'from-primary/80 to-primary-dark', 'from-primary/65 to-primary', 'from-primary to-primary-dark', 'from-primary/90 to-primary-dark'][i] : ['from-ds-blue-400 to-ds-blue-600', 'from-ds-indigo-400 to-ds-indigo-600', 'from-ds-blue-300 to-ds-blue-500', 'from-ds-blue-500 to-ds-indigo-500', 'from-ds-indigo-400 to-ds-blue-600', 'from-ds-blue-400 to-ds-indigo-500', 'from-ds-indigo-500 to-ds-blue-500'][i]}`} />
                        </div>
                    ))}
                </div>
                {/* Day labels */}
                ...
                {isThemeModalOpen && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-auto">
                        ...
                        <motion.div initial={{ opacity: 0, scale: 0.7, y: 40 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.7, y: 40 }}
                            ...
                            <div className="space-y-2.5">
                                {[{ value: 'light', label: 'Light', icon: 'light_mode', color: 'text-primary' }, { value: 'dark', label: 'Dark', icon: 'dark_mode', color: 'text-ds-blue-400' }, { value: 'colorful', label: 'Colorful', icon: 'palette', color: 'text-primary' }].map(t => {
...
function BottomNav({ activeTab, setActiveTab, os, theme }: { activeTab: string, setActiveTab: (t: TabType) => void, os: string, theme: string }) {
    ...
    const navClass = isIOS
        ...
        : (isLight ? 'bg-ds-gray-100 h-20 pb-2 border-t border-ds-blue-100/50' : isColorful ? 'bg-ds-card-colorful-from/80 h-20 pb-2 border-t border-primary/20' : 'bg-ds-dark-3 h-20 pb-2 border-t border-ds-gray-600/50');
...
function NavBtn({ icon, label, active, onClick, os, theme }: { icon: string, label: string, active: boolean, onClick: () => void, os: string, theme: string }) {
    ...
    const isColorful = theme === 'colorful';
    if (isIOS) {
        const activeColor = isColorful ? 'text-primary' : 'text-ds-blue-500';
        return (
    ...
    const iconColor = active ? (isLight ? 'text-ds-blue-900' : isColorful ? 'text-accent' : 'text-ds-blue-200') : (isLight ? 'text-ds-gray-600' : 'text-ds-gray-300');
    return (
            ...
            <div className="relative w-16 h-8 rounded-full flex items-center justify-center">
                {active && (
                    <motion.div
                        ...
                        className={`absolute inset-0 rounded-full ${isColorful ? (isLight ? 'bg-primary/25' : 'bg-primary/25') : (isLight ? 'bg-ds-blue-100' : 'bg-ds-blue-700')}`}
