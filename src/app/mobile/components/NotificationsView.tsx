...
export function NotificationsView({ card, isLight, isColorful = false, theme }: NotificationsViewProps) {
    ...
    return (
        <motion.div initial="hidden" animate="show" exit={{ opacity: 0, x: -20 }} variants={stagger} className={`absolute inset-0 overflow-y-auto scrollbar-none pb-28 ${theme.contentPaddingTop} px-4 space-y-3`}>
            <motion.div variants={fadeUp} className="flex justify-between items-center px-1 mb-1"><h2 className={headerAccentClass}>Notifications</h2><span className={`text-xs font-bold px-2.5 py-1 rounded-full border ${badge('critical')}`}>1 urgent</span></motion.div>
