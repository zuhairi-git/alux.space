...
export function CopilotView({ isLight, isColorful = false, theme }: CopilotViewProps) {
    ...
    const bb = isColorful
        ? `${theme.platform === 'ios' ? 'bg-[#0a0600]/60 backdrop-blur-[20px] border border-[#ff8c42]/20 text-white rounded-[22px] rounded-tl-sm shadow-sm' : 'bg-[#0b0b13]/40 backdrop-blur-xl border border-[#ff8c42]/20 text-white rounded-[22px] rounded-tl-sm shadow-sm'}`
    ...
    const promptCardClass = isColorful
        ? `${theme.platform === 'ios' ? 'rounded-[16px]' : 'rounded-[18px]'} bg-purple-500/10 text-white backdrop-blur-xl border border-[#ff8c42]/20`
