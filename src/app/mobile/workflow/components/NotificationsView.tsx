            ...
            {alerts.map(a => (<motion.div key={a.id} variants={fadeUp} onClick={() => setExp(exp === a.id ? null : a.id)} className={`p-5 ${card} active:scale-[0.98] cursor-pointer`}>
                <div className="flex items-start space-x-3.5">
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-1.5"><h3 className="font-semibold text-[15px] truncate pe-2">{a.title}</h3><span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border shrink-0 ${badge(a.priority)}`}>{a.priority}</span></div>
