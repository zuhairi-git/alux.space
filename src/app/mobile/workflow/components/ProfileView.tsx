...
interface ProfileViewProps {
    ...
    isColorful?: boolean;
    themeMode: string;
    setThemeMode: (t: 'dark' | 'light' | 'colorful') => void;
    theme: MobileTheme;
}
...
export function ProfileView({ card, isLight, isColorful = false, themeMode, setThemeMode, theme }: ProfileViewProps) {
    ...
    const modalBgClass = isColorful
        ? `${theme.radii.modal} p-6 shadow-2xl bg-ds-card-colorful-from text-white`
        ...
        <motion.div initial="hidden" animate="show" exit={{ opacity: 0, x: -20 }} variants={stagger} className={`absolute inset-0 overflow-y-auto scrollbar-none pb-28 ${theme.contentPaddingTop} px-5 space-y-6`}>
            ...
            <motion.div variants={fadeUp} className="flex flex-col items-center py-6">
                <div className="relative mb-4">
                    <div className={`w-28 h-28 rounded-full overflow-hidden p-[3px] bg-gradient-to-tr ${isColorful ? 'from-primary to-primary-dark' : 'from-primary-500 to-primary-300'} shadow-lg`}>
                        <div className={`w-full h-full rounded-full overflow-hidden relative ${isLight ? 'bg-ds-gray-50' : 'bg-ds-dark-1/80'}`}>
                            <Image
                    ...
                    <div className={`absolute bottom-0.5 end-0.5 w-7 h-7 bg-[${theme.accent.success}] border-[3px] rounded-full ${theme.platform === 'android' ? (isLight ? 'border-ds-gray-50' : 'border-ds-dark-1') : (isLight ? 'border-ds-gray-100' : 'border-ds-dark-1')}`} />
                </div>
