...
export const componentRegistry: ComponentEntry[] = [
  // ─── Primitives ─────────────────────────────────────────
  {
    ...
    props: [
      ...
      { name: 'size', type: "'xs' | 'sm' | 'md' | 'lg' | 'xl' | '2xl' | 'display'", default: "'md' via CSS default", description: 'Semantic icon size with optical-size pairing' },
      { name: 'variant', type: "'outline' | 'filled'", default: "'outline'", description: 'Material Symbols fill axis' },
      { name: 'tone', type: "'current' | 'default' | 'muted' | 'primary' | 'accent' | 'inverse'", default: 'undefined', description: 'Theme-resolved semantic icon color' },
      { name: 'purpose', type: "'decorative' | 'standalone'", default: "'decorative'", description: 'Accessibility behavior for standalone icons' },
      { name: 'label', type: 'string', default: 'undefined', description: 'Required accessible label for standalone icons' },
      { name: 'className', type: 'string', default: "''", description: 'Additional CSS classes' },
    ],
    tokens: ['--material-symbols', '--icon-size-*', '--primary', '--accent-text', '--foreground', '--muted-foreground', '--text-on-primary'],
    a11y: ['aria-hidden for decorative icons', 'aria-label + role="img" for standalone icons'],
    variants: ['size scale', 'outline', 'filled', 'semantic tones'],
  },
  {
    name: 'IconSurface',
    path: 'src/components/ui/Icon.tsx',
    ...
    props: [
      { name: 'name', type: 'string', default: 'required', description: 'Material Symbols icon name' },
      { name: 'surfaceSize', type: "'xs' | 'sm' | 'md' | 'lg' | 'xl'", default: "'md'", description: 'Tokenized icon container size' },
      { name: 'surfaceTone', type: "'neutral' | 'primary' | 'accent' | 'inverse'", default: 'undefined', description: 'Theme-resolved boxed icon styling' },
      { name: 'shape', type: "'square' | 'circle'", default: "'square'", description: 'Container shape for boxed icons' },
      { name: 'size', type: "IconSize", default: 'surface-matched', description: 'Optional glyph size override inside the surface' },
      { name: 'tone', type: "IconTone", default: 'surface-matched', description: 'Optional glyph tone override' },
      { name: 'className', type: 'string', default: "''", description: 'Additional container CSS classes' },
    ],
    tokens: ['--icon-surface-size-*', '--icon-surface-radius-*', '--primary', '--accent-text', '--foreground', '--text-on-primary'],
    a11y: ['inherits Icon accessibility behavior'],
    variants: ['surface size scale', 'neutral', 'primary', 'accent', 'inverse', 'square', 'circle'],
  },
  {
    name: 'MaterialSymbol',
  ...
  },
  {
    ...
    category: 'section',
    props: [
      { name: 'animation', type: "'fade-in' | 'slide-up' | 'slide-down'", default: "'fade-in'", description: 'Animation type' },
      { name: 'delay', type: 'number', default: '0', description: 'Delay (seconds)' },
      { name: 'duration', type: 'number', default: '0.5', description: 'Duration (seconds)' },
    ...
    tokens: ['--duration-slow', '--ease-out', '--motion-reveal-offset'],
    a11y: ['role attribute', 'aria-label'],
    variants: ['fade-in', 'slide-up', 'slide-down'],
  },
  {
    name: 'PodcastPlayer',
  ...
  },
  {
    ...
    category: 'primitive',
    props: [
      { name: 'variant', type: "'primary' | 'secondary' | 'tertiary' | 'icon' | 'ghost' | 'outline' | 'cosmic' | 'glass' | 'overlay'", default: "'primary'", description: 'Button emphasis level. primary → filled; secondary → outlined; tertiary → text-only; icon → square icon button.' },
      { name: 'size', type: "'sm' | 'md' | 'lg'", default: "'md'", description: 'Tokenized button size. Inline icons and icon-only buttons inherit matching DS icon sizes automatically.' },
  ...
  },
  {
    ...
    props: [
      { name: 'name', type: 'string', default: 'required', description: 'Material Symbols icon name' },
      { name: 'buttonSize', type: "'sm' | 'md' | 'lg'", default: "'md'", description: 'Match icon size to the parent button size' },
      { name: 'emphasis', type: "'inline' | 'icon-only'", default: "'inline'", description: 'Use inline sizing for text buttons or larger glyph sizing for icon-only buttons' },
      { name: 'tone', type: "IconTone", default: "'current'", description: 'Inherit or override icon color inside buttons' },
    ],
    tokens: ['--btn-content-icon-size-*', '--btn-icon-only-icon-size-*'],
    a11y: ['Use with Button aria-label rules for icon-only buttons'],
    variants: ['inline', 'icon-only'],
  },
  {
    name: 'Badge',
    ...
    category: 'primitive',
    props: [
      { name: 'variant', type: "'default' | 'success' | 'warning' | 'error' | 'info' | 'outline' | 'gradient' | 'glass' | 'overlay' | 'accent'", default: "'default'", description: 'Badge color/style variant' },
    ...
    tokens: [
      '--primary', '--radius-full', '--font-size-xs',
      ...
      '--badge-glass-bg', '--badge-glass-border', '--badge-glass-text',
      '--badge-overlay-bg', '--badge-overlay-border',
      '--badge-accent-bg', '--badge-accent-border', '--badge-accent-text',
    ],
    a11y: ['role="status"'],
    variants: ['default', 'success', 'warning', 'error', 'info', 'outline', 'gradient', 'glass', 'overlay', 'accent'],
  },
  {
    name: 'Input',
