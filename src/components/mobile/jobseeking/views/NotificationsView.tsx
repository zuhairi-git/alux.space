                ...
                <motion.div variants={listVariants} initial="hidden" animate="visible" className="space-y-3">
                    ...
                    <motion.div variants={itemVariants} className={`${card} overflow-hidden`}>
                        <div className={`px-4 py-2 flex items-center gap-2 bg-ds-success/10 border-b ${isLight ? 'border-ds-success/15' : 'border-ds-success/20'}`}>
                            ...
                            <span className={`ml-auto text-[11px] ${isLight ? 'text-ds-gray-400' : 'text-white/40'}`}>Just now</span>
                        </div>
                        <div className="p-4">
                            ...
                            <div className="flex gap-2 mt-3">
                                <button onClick={() => onNav?.('copilot')}
                                    className={`flex-1 py-2 rounded-xl text-[12px] font-semibold flex items-center justify-center gap-1.5 ${isLight ? 'bg-black/5 text-ds-gray-700' : 'bg-white/8 text-white/80'} active:scale-95 transition-transform`}>
                    ...
                    <motion.div variants={itemVariants} className={`${card} overflow-hidden`}>
                        <div className={`px-4 py-2 flex items-center gap-2 ${isColorful ? 'bg-primary/10 border-b border-primary/20' : isLight ? 'bg-primary/8 border-b border-[var(--primary)]/10' : 'bg-primary/10 border-b border-[var(--primary)]/15'}`}>
                            <Icon name="auto_awesome" className={`text-[13px] text-primary`} />
                            <span className={`text-[11px] font-bold uppercase tracking-wider text-primary`}>New AI Match</span>
                            <span className={`ml-auto text-[11px] ${isLight ? 'text-ds-gray-400' : 'text-white/40'}`}>2h ago</span>
                        </div>
                        <div className="p-4">
                            <div className="flex items-start gap-3">
                            ...
                            <button onClick={() => onNav?.('jobs')}
                                className={`mt-3 w-full py-2 rounded-xl text-[12px] font-semibold flex items-center justify-center gap-1.5 ${isLight ? 'bg-black/5 text-ds-gray-700' : 'bg-white/8 text-white/80'} active:scale-95 transition-transform`}>
                    ...
                    <motion.div variants={itemVariants} className={`${card} overflow-hidden`}>
                        <div className={`px-4 py-2 flex items-center gap-2 ${isLight ? 'bg-primary/5 border-b border-primary/10' : 'bg-primary/10 border-b border-primary/15'}`}>
                            ...
                            <span className={`ml-auto text-[11px] ${isLight ? 'text-ds-gray-400' : 'text-white/40'}`}>1d ago</span>
                        </div>
                        <div className="p-4">
                            <div className="grid grid-cols-3 gap-2 mb-3">
                                {[
                                    { label: 'Profile Views', value: '+20%', icon: 'visibility', color: 'text-primary' },
                                    { label: 'Applications', value: '4', icon: 'send', color: 'text-primary' },
                                    ...
                                    <div key={stat.label} className={`flex flex-col items-center py-2.5 rounded-xl ${isLight ? 'bg-black/[0.03]' : 'bg-white/[0.04]'}`}>
                                        ...
                                        <span className={`text-[10px] text-center leading-tight mt-0.5 ${isLight ? 'text-ds-gray-500' : 'text-white/40'}`}>{stat.label}</span>
                                    </div>
                                ))}
                            </div>
                            <p className={`text-[13px] leading-snug ${isLight ? 'text-ds-gray-500' : 'text-ds-gray-400'}`}>
