...
export default function PromptPage() {
  ...
  const getDropdownItemStyles = (isSelected: boolean) => {
    const baseClasses = 'w-full text-start px-4 py-3 text-sm transition-all duration-200';

    if (isSelected) {
      if (theme === 'light')
        return `${baseClasses} bg-gradient-to-r from-ds-blue-500/20 to-primary-500/20 text-ds-blue-700 font-medium border-s-2 border-ds-blue-500`;
      if (theme === 'colorful')
  ...
  const getStatusDropdownItemStyles = (isSelected: boolean) => {
    ...
    if (isSelected) {
      if (theme === 'light')
        return `${baseClasses} bg-gradient-to-r from-ds-emerald-600/20 to-ds-cyan-500/20 text-ds-emerald-700 font-medium border-s-2 border-ds-emerald-500`;
                ...
                <div className="relative dropdown-container z-[80]">
                  ...
                  <AnimatePresence>
                    {categoryDropdownOpen && (                      <motion.div
                        ...
                        className={`absolute top-full mt-2 start-0 rounded-xl shadow-2xl z-[80] min-w-[180px] overflow-hidden ${getDropdownMenuStyles()}`}
                ...
                <div className="relative dropdown-container z-[80]">
                  ...
                  <AnimatePresence>
                    {statusDropdownOpen && (                      <motion.div
                        ...
                        exit={{ opacity: 0, y: -10, scale: 0.95 }}
                        transition={{ duration: 0.2 }}
                        className={`absolute top-full mt-2 start-0 rounded-xl shadow-2xl z-[80] min-w-[140px] overflow-hidden ${getDropdownMenuStyles()}`}
                      >                        {statuses.map((status) => (
                          <motion.button
                            key={status}
                            whileHover={getDropdownHoverEffect()}
        ...
        <div className="flex items-start gap-6">
          ...
          <div className="flex-1">
            <div className="flex items-start justify-between mb-3">
              ...
              <div className={`px-4 py-2 rounded-full text-sm font-bold text-white bg-gradient-to-r ${getStatusColor(prompt.status)} shadow-lg shrink-0 ms-4`}>
                {prompt.status}
              </div>            </div>
