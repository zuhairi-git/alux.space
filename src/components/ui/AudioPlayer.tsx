...
const AudioPlayer: React.FC<AudioPlayerProps> = ({ src, title, category }) => {
  ...
  const [dropdownPosition, setDropdownPosition] = useState<{
    ...
    // Enhanced mobile states with iOS-specific handling
  const [isTouching, setIsTouching] = useState(false);
  const [lastTap, setLastTap] = useState(0);
  const [iosRetryCount, setIosRetryCount] = useState(0);
  ...
  const styles = getThemeStyles();  // Detect iOS/Safari for specific handling
  const isIOS = useMemo(() => {
  ...
  useEffect(() => {
    console.log('AudioPlayer: Initializing with src:', src, { isIOS, isSafari });
  ...
  const togglePlay = async () => {
    ...
    console.log('AudioPlayer: Toggle play, current state:', isPlaying ? 'playing' : 'paused', { isIOS, isSafari });
      ...
      if (isIOS || isSafari) {
        try {
          ...
          if (audioRef.current.readyState === 0) {
            console.log('AudioPlayer: Loading audio for iOS/Safari');
          ...
          await playPromise;
          setIsPlaying(true);
          console.log('AudioPlayer: Playing successfully on iOS/Safari');
          
        } catch (error) {
          console.error("AudioPlayer: iOS/Safari play failed:", error);
          setLoadError(true);
        }
      } else {
  ...
  const retryLoading = () => {
    ...
    console.log('AudioPlayer: Retrying audio load, attempt:', iosRetryCount + 1, { isIOS, isSafari });
    setLoadError(false);
    setIsPlaying(false);
    setCurrentTime(0);
    setDuration(0);
      ...
      if (isIOS || isSafari) {
        ...
        setTimeout(() => {
          if (audioRef.current) {
            ...
            audioRef.current.load();
            console.log('AudioPlayer: iOS retry load initiated with delay');
          }
        }, 100);
      } else {
        audioRef.current.src = src;
        audioRef.current.load();
        console.log('AudioPlayer: Standard retry load initiated');
      }
    } catch (err) {
  ...
  return (    <motion.div      initial={animationsDisabled ? {} : { opacity: 0, y: 20 }}
    ...
    >      {/* Dynamic background effects */}
      <div className="absolute inset-0 overflow-hidden opacity-20 pointer-events-none rounded-2xl" style={{ zIndex: -1 }}>
        ...
        />
      </div>      {/* Hidden audio element with iOS Safari optimizations */}
      <audio 
        ref={audioRef} 
